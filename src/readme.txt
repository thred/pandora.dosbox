...
This is a compilation of the DOSBox v0.74-2 as released on August 30th, 2018.

Dynamic recompile is enabled. Screenshots and movie recording is disabled.

The launch script will ask you for the directory, that should act as C drive.
It does this everytime it starts (press enter, enter, if it is the same as 
last time). It places a dosbox.conf file within this directory (if none exists)
and uses it. That's how you are able to configure the DOSBox for this specific 
application.

The keyboard mapping is still missing. To type a ":", hit and hold "Shift" "Fn"
and "," in that order. To type a "?", hit and hold "Shift" "Fn" and "z".

The property "output" should be kept to "overlay", otherwise color fading is 
very slow.

If the sound stutters, try to set "cycles" to "full" instead of "auto".
